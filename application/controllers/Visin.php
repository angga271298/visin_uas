...
class Visin extends CI_Controller {
	public function index()
	{
        ...
        $output['tahunan']=$this->tahunan($data);
    ...
    function sales($data)
    {
        ...
        foreach($data as $row)
        {
            if(isset($result[$row->Agen]) == false)
            {
                $result[$row->Agen]=$row->Units;
            }else{
                $units=$result[$row->Agen];
                $result[$row->Agen]=$units + $row->Units;
            }
        };
    ...
    function produk($data)
    {
        ...
        foreach($data as $row)
        {
            if(isset($result[$row->Mobil]) == false)
            {
                $result[$row->Mobil]=$row->Units;
            }else{
                $units=$result[$row->Mobil];
                $result[$row->Mobil]=$units + $row->Units;
            }
        };
        //sorting data berdasarkan value array secara menurun
        arsort($result);
